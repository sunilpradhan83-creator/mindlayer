...
Do not duplicate memory into `CLAUDE.md` or retrieve durable context from this adapter. Do not write memory without explicit approval. Follow `AGENTS.md` for automatic MindLayer boot; `/m-init` is a legacy/manual refresh alias for showing or rerunning the boot receipt.
