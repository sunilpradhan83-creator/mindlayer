...
mkdir -p "$SANDBOX/archive/.mindlayer"
printf "# Context\n\n## Old Entry\n\nStale content.\n\n## Live Entry\n\nCurrent.\n" \
  > "$SANDBOX/archive/.mindlayer/context.md"
...
if assert_file_contains "$SANDBOX/archive/.mindlayer/context.md" "Old Entry"; then pass "$CURRENT_SCENARIO: no archive before approval"; else fail "$CURRENT_SCENARIO: no archive before approval"; fi
if (cd "$SANDBOX/archive" && python3 "$ROOT_DIR/src/ml" archive \
...
if ! grep -Fq "Old Entry" "$SANDBOX/archive/.mindlayer/context.md"; then pass "$CURRENT_SCENARIO: section removed from source"; else fail "$CURRENT_SCENARIO: section removed from source"; fi
if assert_file_contains "$SANDBOX/archive/.mindlayer/archive.md" "Old Entry"; then pass "$CURRENT_SCENARIO: section in archive.md"; else fail "$CURRENT_SCENARIO: section in archive.md"; fi
if assert_file_contains "$SANDBOX/archive/.mindlayer/archive.md" "Stale content."; then pass "$CURRENT_SCENARIO: content in archive.md"; else fail "$CURRENT_SCENARIO: content in archive.md"; fi
if assert_file_contains "$SANDBOX/archive/.mindlayer/context.md" "Live Entry"; then pass "$CURRENT_SCENARIO: live entry preserved"; else fail "$CURRENT_SCENARIO: live entry preserved"; fi

# --- delete section ---
scenario "delete section"
mkdir -p "$SANDBOX/delete/.mindlayer"
printf "# Context\n\n## Ephemeral\n\nTemp content.\n\n## Keeper\n\nStays.\n" \
  > "$SANDBOX/delete/.mindlayer/context.md"
...
if ! grep -Fq "Ephemeral" "$SANDBOX/delete/.mindlayer/context.md"; then pass "$CURRENT_SCENARIO: section removed from source"; else fail "$CURRENT_SCENARIO: section removed from source"; fi
if ! [ -f "$SANDBOX/delete/.mindlayer/archive.md" ] || ! grep -Fq "Ephemeral" "$SANDBOX/delete/.mindlayer/archive.md"; then
...
mkdir -p "$SANDBOX/missing/.mindlayer"
printf "# Context\n\n## Only Entry\n\nContent.\n" > "$SANDBOX/missing/.mindlayer/context.md"
...
mkdir -p "$SANDBOX/selfarch/.mindlayer"
printf "# Archive\n\n## Old\n\nArchived.\n" > "$SANDBOX/selfarch/.mindlayer/archive.md"
...
mkdir -p "$SANDBOX/newarchive/.mindlayer"
printf "# Decisions\n\n## Old Decision\n\nWas decided.\n" > "$SANDBOX/newarchive/.mindlayer/decisions.md"
...
if assert_file_contains "$SANDBOX/newarchive/.mindlayer/archive.md" "# Archive"; then pass "$CURRENT_SCENARIO: archive.md has heading"; else fail "$CURRENT_SCENARIO: archive.md has heading"; fi
